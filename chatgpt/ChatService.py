...
class ChatService:
    ...
    async def prepare_send_conversation(self):
        try:
            ...
            raise HTTPException(status_code=422, detail="Failed to format messages.Temporary chage 400 status code to 422")
    ...
    async def upload_file(self, file_content, mime_type, max_retries=3, retry_delay=1):
        ...
        if mime_type.startswith("image/"):
            try:
                ...
                logger.error(f"Error getting image size, change to text/plain: {e}")
